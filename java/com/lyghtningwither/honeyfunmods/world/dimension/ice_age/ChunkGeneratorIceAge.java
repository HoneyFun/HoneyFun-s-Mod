...
public class ChunkGeneratorIceAge implements IChunkGenerator
{
    ...
    public ChunkGeneratorIceAge(World world, boolean mapFeaturesEnabled, long seed, BlockPos spawn)
    {
        this.world = world;
        ...
        spawnableCreatureList.add(new SpawnListEntry(EntityCaveSpider.class, 1, 1, 1));
        spawnableCreatureList.add(new SpawnListEntry(EntityPolarBear.class, 5, 1, 2));
    ...
    public static float getIslandHeightValue(int x, int z, int p_185960_3_, int p_185960_4_)
    {
        ...
        for (int i = -12; i <= 12; ++i)
        {
            for (int j = -12; j <= 12; ++j)
            {
                ...
                if (k * k + l * l > 4096L && ChunkGeneratorIceAge.islandNoise.getValue((double)k, (double)l) < -0.8999999761581421D)
                {
